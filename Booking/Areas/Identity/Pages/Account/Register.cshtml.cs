...
namespace Booking.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private BookingUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<BookingUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(BookingUser)}'. " +
                    $"Ensure that '{nameof(BookingUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<BookingUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<BookingUser>)_userStore;
        }
    }
}
